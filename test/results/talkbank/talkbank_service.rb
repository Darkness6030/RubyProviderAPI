...
# Сгенерировано из TalkBank BaaS. Перед production-использованием проверьте маппинг полей.
...
class Provider
  class TalkbankService < BaseService
    BASE_URL = ENV.fetch("TALKBANK_BASE_URL", "https://baas.staging.talkbank.dev/api/v1")
    STATUS_MAP = {"new" => "in_progress"}.freeze
    ERROR_MAP = {400 => "bad_request", 403 => "forbidden", 404 => "bad_request", 422 => "unprocessable_entity", 500 => "internal_server_error"}.freeze
    ...
    def create_request(operation, request_method = nil)
      ...
      path = expand_path("/account/transfer", operation)
    ...
    def fetch_status(operation)
      ...
      path = expand_path("/account/transfer/{order_slug}", operation)
