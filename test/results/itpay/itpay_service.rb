...
# Сгенерировано из ITPAY API. Перед production-использованием проверьте маппинг полей.
...
class Provider
  class ItpayService < BaseService
    BASE_URL = ENV.fetch("ITPAY_BASE_URL", "https://api.gw.itpay.ru")
    STATUS_MAP = {"new" => "in_progress", "paid" => "approved", "processing" => "in_progress", "completed" => "approved", "cancelled" => "rejected", "rejected" => "rejected", "error" => "rejected", "paying" => "in_progress", "bill" => "in_progress", "deleted" => "rejected"}.freeze
    ERROR_MAP = {400 => "bad_request", 401 => "unauthorized", 403 => "forbidden", 404 => "bad_request", 500 => "internal_server_error"}.freeze
    ...
    def create_request(operation, request_method = nil)
      ...
      path = expand_path("/v1/payments", operation)
    ...
    def fetch_status(operation)
      ...
      path = expand_path("/v1/payments/{id}", operation)
    ...
    def create_headers(operation)
      ...
      # TODO: реализуйте авторизацию типа basic.
