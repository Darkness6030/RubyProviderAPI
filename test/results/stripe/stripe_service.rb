...
# Сгенерировано из Stripe API. Перед production-использованием проверьте маппинг полей.
...
class Provider
  class StripeService < BaseService
    BASE_URL = ENV.fetch("STRIPE_BASE_URL", "https://api.stripe.com/")
    STATUS_MAP = {"paid" => "approved", "pending" => "in_progress", "in_transit" => "in_progress", "canceled" => "rejected", "failed" => "rejected"}.freeze
    ERROR_MAP = {}.freeze
    ...
    def create_request(operation, request_method = nil)
      ...
      path = expand_path("/v1/payouts", operation)
    ...
    def fetch_status(operation)
      ...
      path = expand_path("/v1/payouts/{payout}", operation)
    ...
    def create_headers(operation)
      ...
      credentials = [ENV.fetch("STRIPE_USERNAME"), ENV.fetch("STRIPE_PASSWORD")].join(":")
