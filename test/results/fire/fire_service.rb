...
# Сгенерировано из Fire Financial Services Business API. Перед production-использованием проверьте маппинг полей.
...
class Provider
  class FireService < BaseService
    BASE_URL = ENV.fetch("FIRE_BASE_URL", "https://api.fire.com/business")
    STATUS_MAP = {"PENDING_APPROVAL" => "in_progress", "REJECTED" => "rejected", "COMPLETE" => "approved", "OPEN" => "in_progress", "CANCELLED" => "rejected", "PENDING_PARENT_BATCH_APPROVAL" => "in_progress", "READY_FOR_PROCESSING" => "in_progress", "PROCESSING" => "in_progress"}.freeze
    ERROR_MAP = {400 => "bad_request", 401 => "unauthorized", 403 => "forbidden"}.freeze
    ...
    def create_request(operation, request_method = nil)
      ...
      path = expand_path("/v1/batches", operation)
    ...
    def fetch_status(operation)
      ...
      path = expand_path("/v1/batches/{batchUuid}", operation)
    ...
    def create_headers(operation)
      ...
      headers["Authorization"] = "Bearer #{ENV.fetch("FIRE_ACCESS_TOKEN")}"
