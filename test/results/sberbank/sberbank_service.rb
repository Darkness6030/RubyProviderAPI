...
# Сгенерировано из API платёжного шлюза Сбербанка. Перед production-использованием проверьте маппинг полей.
...
class Provider
  class SberbankService < BaseService
    BASE_URL = ENV.fetch("SBERBANK_BASE_URL", "https://ecomtest.sberbank.ru")
    STATUS_MAP = {"1" => "unknown", "0" => "unknown"}.freeze
    ERROR_MAP = {400 => "bad_request", 404 => "bad_request", 405 => "internal_server_error", 429 => "too_many_requests", 451 => "internal_server_error", 500 => "internal_server_error"}.freeze
    ...
    def create_request(operation, request_method = nil)
      ...
      path = expand_path("/ecomm/gw/partner/api/p2p/v1/register", operation)
    ...
    def check_conditions(operation, request_method)
      ...
      return failure(:unprocessable_entity, "missing_userName") if value_missing?(read_path(payload, "userName"))
      return failure(:unprocessable_entity, "missing_password") if value_missing?(read_path(payload, "password"))
      return failure(:unprocessable_entity, "missing_orderNumber") if value_missing?(read_path(payload, "orderNumber"))
    ...
    def create_headers(operation)
      ...
      headers["x-idempotencyKey"] = operation.id.to_s if operation.respond_to?(:id)
      headers
    end
