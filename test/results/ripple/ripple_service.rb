...
# Сгенерировано из Payments Direct API. Перед production-использованием проверьте маппинг полей.
...
class Provider
  class RippleService < BaseService
    BASE_URL = ENV.fetch("RIPPLE_BASE_URL", "https://api.test.ripple.com")
    STATUS_MAP = {"INITIATED" => "in_progress", "QUOTED" => "in_progress", "VALIDATING" => "in_progress", "TRANSFERRING" => "in_progress", "COMPLETED" => "approved", "FAILED" => "rejected", "RETURNED" => "rejected", "DECLINED" => "rejected", "AWAITING_FUNDING" => "in_progress"}.freeze
    ERROR_MAP = {400 => "bad_request", 401 => "unauthorized", 402 => "unprocessable_entity", 403 => "forbidden", 404 => "bad_request", 405 => "internal_server_error", 409 => "unprocessable_entity", 415 => "internal_server_error", 422 => "unprocessable_entity", 500 => "internal_server_error"}.freeze
    ...
    def create_request(operation, request_method = nil)
      ...
      path = expand_path("/v3/payments", operation)
    ...
    def fetch_status(operation)
      ...
      path = expand_path("/v3/payments/{paymentId}", operation)
    ...
    def create_headers(operation)
      ...
      headers["Authorization"] = "Bearer #{ENV.fetch("RIPPLE_ACCESS_TOKEN")}"
